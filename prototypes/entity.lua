...
local function def_CC_table(main_offset, shadow_offset, variation)
    return { variation = variation, main_offset = main_offset, shadow_offset = shadow_offset, show_shadow = shadow_offset ~= nil 
        }
end
...
local function logistic_container(entity_name, item_name, icon_name, picture_name, additional_pastable_entities, inventory_size, logistic_mode)
...
local function heat_pipe(entity_name, item_name, tint, use_heated_pictures, max_temperature)
	...
	for _, pictures in pairs(heat_pipe.connection_sprites) do
		for _, picture in ipairs(pictures) do
			...
			if use_heated_pictures then
				...
				if not filename:find("heat%-pipe%-straight%-horizontal%-single.png") and not filename:find("heat%-pipe%-straight%-vertical%-single.png") then
			...
			if hr_version then
				if use_heated_pictures then
					...
					if not filename:find("hr%-heat%-pipe%-straight%-horizontal%-single.png") and not filename:find("hr%-heat%-pipe%-straight%-vertical%-single.png") then
	...
	heat_pipe.fast_replaceable_group = heat_pipe.fast_replaceable_group or creative_mode_defines.names.fast_replaceable_groups.heat_source_void
	return heat_pipe
end

-- Generates data for inserter according to the given data.
local function inserter(entity_name, item_name, icon_name, platform_sheet_name, additional_pastable_entities, fast_replaceable_group, pickup_position, insert_position, filter_count)
...
local function electric_energy_interface(entity_name, item_name, icon_name, animation_filename, usage_priority, default_input_flow, default_output_flow, default_production, default_usage, additional_pastable_entities)
	return
	{
...
circuit_connector_definitions[creative_mode_defines.names.entities.super_roboport] = circuit_connector_definitions.create
    (
      universal_connector_template,
      {
...
circuit_connector_definitions[creative_mode_defines.names.entities.fluid_void] = circuit_connector_definitions.create
    (
      universal_connector_template,
      {
        def_CC_table({-0.1875, -0.375}, nil, 7),
...
circuit_connector_definitions[creative_mode_defines.names.entities.passive_energy_source] = circuit_connector_definitions.create
    (
      universal_connector_template,
      {
...
circuit_connector_definitions[creative_mode_defines.names.entities.passive_energy_void] = circuit_connector_definitions.create
    (
      universal_connector_template,
      {
        def_CC_table({0.46875, 0.5}, {0.46875, 0.8125}, 26),
...
{	
	...
	container(
		...
		"duplicating-chest.png",
		{creative_mode_defines.names.entities.duplicating_provider_chest, creative_mode_defines.names.entities.duplicating_cargo_wagon},
	...
	logistic_container(
		...
		"duplicating-provider-chest.png",
		{creative_mode_defines.names.entities.duplicating_chest, creative_mode_defines.names.entities.duplicating_cargo_wagon},
		settings.startup[creative_mode_defines.names.settings.duplicating_chest_size].value,
		"passive-provider"),
	...
	cargo_wagon(
		...
		{r = 0.35, g = 0.3, b = 1},
		{creative_mode_defines.names.entities.duplicating_chest, creative_mode_defines.names.entities.duplicating_provider_chest},
	...
	super_boiler(
		...
		"configurable-super-boiler.png",
		{creative_mode_defines.names.entities.configurable_super_boiler}), -- This is needed because boiler does not support copy-and-paste by default.
	...
	electric_energy_interface(
		...
		"0kW", -- Default usage
		{creative_mode_defines.names.entities.passive_electric_energy_interface, creative_mode_defines.names.entities.active_electric_energy_interface_input}),
	...
	electric_energy_interface(
		...
		"0kW",
		"500GW",
		{creative_mode_defines.names.entities.active_electric_energy_interface_output, creative_mode_defines.names.entities.active_electric_energy_interface_input}),
	...
	electric_energy_interface(
		...
		"0kW",
		"500GW",
		{creative_mode_defines.names.entities.active_electric_energy_interface_output, creative_mode_defines.names.entities.passive_electric_energy_interface}),
	...
	magic_wand_smoke(
		creative_mode_defines.names.entities.magic_wand_smoke_creator,
		{r = 1, g = 0, b = 0, a = 0.4}),
		
	-- Magic wand smoke - healer
	magic_wand_smoke(
		creative_mode_defines.names.entities.magic_wand_smoke_healer,
		{r = 0, g = 1, b = 0, a = 0.4}),
		
	-- Magic wand smoke - modifier
	magic_wand_smoke(
		creative_mode_defines.names.entities.magic_wand_smoke_modifier,
		{r = 0.5, g = 0, b = 1, a = 0.4}),
