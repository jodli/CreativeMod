...
local function create_team_target_auto_apply_cheat_elements_from_data(cheats_container, cheat_gui_data, targets)
	...
	-- Drop down inner frame.
	local drop_down_inner_frame =
		drop_down_scroll_pane.add {
		type = "frame",
	...
	if targets_scroll_pane then
		...
		if targets_inner_container then
			...
			if special_target ~= nil then
				...
				local button_name =
					button_name_prefix .. cheats_menu_gui_data.get_target_button_name_postfix_function(player, special_target)
				local button = targets_inner_container[button_name]
				if button then
					local button_style_name = button.style.name
					if
						check_style and button_style_name ~= creative_mode_defines.names.gui_styles.cheat_target_selected_button and
		...
		if cheats_menus_container then
			...
			elseif not destroy_only then
					...
					if unverified_target_list then
						for _, target in pairs(unverified_target_list) do
							...
							if cheats_menu_gui_data.verify_target_function(player, target) then
								local button, is_self =
									add_target_button_in_targets_inner_container_for_player(
									player,
									target,
									cheats_menu_gui_data,
									targets_inner_container,
				...
				local cheats_container =
					cheats_scroll_pane.add {type = "flow", name = current_structure_data.name, direction = "vertical"}
...
local function update_targets_selection_for_player_in_outer_container(
	...
	if current_structure_data then
		...
		for _, child_name in ipairs(targets_inner_container.children_names) do
			...
			local button_style_name = button.style.name
			if
				button_style_name == creative_mode_defines.names.gui_styles.cheat_target_self_selected_button or
	...
	if container then
		...
		if cheats_menus_container then
			...
			if frame then
				...
				local selected_targets =
					update_targets_selection_for_player_in_outer_container(
					player,
					cheats_menu_gui_data,
					outer_container_data,
					outer_container
			...
			if inner_container then
				...
				for _, child_name in ipairs(inner_container.children_names) do
					...
					if cheats_menu_gui_data.check_is_target_button_valid_unknown_target_function(player, button) then
						local style_name = button.style.name
						if
							style_name == creative_mode_defines.names.gui_styles.cheat_target_unselected_button or
								style_name == creative_mode_defines.names.gui_styles.cheat_target_self_unselected_button
						 then
							deselect_all = false
							break
						end
		...
		if cheats_menu_gui_data.get_button_actual_target_function then
			...
			if target then
				...
				if shift then
					...
					if
						global.last_cheats_menu_gui_actions_by_players and global.last_cheats_menu_gui_actions_by_players[player.index] and
							global.last_cheats_menu_gui_actions_by_players[player.index].non_shift
					 then
						...
						for index, children in ipairs(inner_container.children) do
							...
							if
								style_name == creative_mode_defines.names.gui_styles.cheat_target_selected_button or
									style_name == creative_mode_defines.names.gui_styles.cheat_target_self_selected_button
							 then
