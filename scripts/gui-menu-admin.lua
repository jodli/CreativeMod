...
local function create_access_right_elements_from_data(access_rights_container, access_right_gui_data)
	...
	-- Label.
	local label =
		container.add {type = "label", name = access_right_gui_data.label_name, caption = access_right_gui_data.label_caption}
	if access_right_gui_data.is_overall then
		label.style = "caption_label"
	end
	-- Inner container.
	local inner_container =
		container.add {type = "table", name = access_right_gui_data.inner_container_name, column_count = 1}
...
function gui_menu_admin.create_or_destroy_menu_for_player(player)
	...
	if container then
		...
		if admin_menus_container then
			...
			-- Access right frame.
			local access_right_frame =
				admin_menus_container.add {
				type = "frame",
			...
			-- Scroll-pane.
			local access_rights_scorll_pane =
				access_right_frame.add {
				type = "scroll-pane",
			...
			local disable_creative_mode_frame =
				admin_menus_container.add {
				type = "frame",
