...
function item_providers_util.output_or_remove_item(surface, position, shift_x, shift_y, direction, filter_item_name, operation_mode, output_item_slot, entity_data)
	...
	if operation_mode == output_or_remove_item_operation_mode.output_mode and filter_item_name then
		output_stack = {name = filter_item_name, count = 1}
	end
	-- Add the offset to the position to get the actual one.
	local actual_position = {x = position.x + shift_x, y = position.y + shift_y}
	...
	if transport_belt_entity == nil and other_inventory_entity == nil and other_fluidbox_entity == nil then
		...
		for _, entity in ipairs(surface.find_entities_filtered{position = actual_position, force = entity_data.entity.force}) do	
			...
			elseif entity.type == "character" then
				...
				if not collision_mask or not util.array_contains(collision_mask, "not-colliding-with-itself") then
					if util.has_inventory(entity) then
						-- Ignore construction robots and logistic robots. They are innocent.
						if entity.type ~= "construction-robot" and entity.type ~= "logistic-robot" and entity.type ~= "rocket-silo-rocket" then
				...
				if entity_data.last_working_static_container_type == static_item_container_type.crafting_machine then
					...
					if other_inventory_entity.get_recipe() then
						...
						if inventories == nil then
							inventories = {other_inventory_entity.get_inventory(defines.inventory.assembling_machine_input)}
				...
				if output_stack then
					...
					if entity_data.insert_only_once_to_player then
						if entity_data.insert_to_player_by_stack then
							output_stack.count = game.item_prototypes[output_stack.name].stack_size * entity_data.insert_to_player_amount
						else
					...
					for _, player in ipairs(player_entities) do
						...
						local not_inserted_into_player = not last_player_entities or not util.array_contains(last_player_entities, player)
