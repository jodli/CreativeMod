...
local function create_tile_or_resource_selection_for_creator(
	...
	local resource_amount_container =
		container.add {
		type = "table",
...
local function create_elements_for_recorded_actions_of_modifier(player, actions_container, container)
	if
		global.creative_mode.modifier_magic_wand_quick_actions[player.index] and
	...
	creator = {
		...
		contents = {
			...
			create_select_mode_gui_function = function(player, container)
				...
				-- Container.
				local tile_or_resource_2_container =
					container.add {
					type = "flow",
	...
	healer = {
		...
		contents = {
			...
			create_alt_select_mode_gui_function = function(player, container)
				...
				local alt_mode_actions_container =
					container.add {
					type = "flow",
	...
	if container then
		...
		if magic_wand_menus_container then
			...
			local magic_wand_frame =
				magic_wand_menus_container.add {
				type = "frame",
		...
		if magic_wand_menus_container then
			...
			if not frame then
				...
				local title_container =
					frame.add {type = "flow", name = contents_data.title_container_name, direction = "horizontal"}
				-- Frame title.
				title_container.add {
					type = "label",
				...
				local alt_select_mode_container =
					flow.add {
					type = "frame",
...
function gui_menu_magicwand.create_or_destroy_modification_popup_for_player(player, is_create)
	...
	if is_create then
		...
		-- Title container.
		local entities_frame_title_container =
			entities_frame.add {
			type = "flow",
		...
		for entity_name, data in pairs(sorted_entities) do
			...
			if count > 0 then
				...
				if item_name then
					...
					button_name =
						creative_mode_defines.names.gui.magic_wand_modifier_popup_item_on_ground_name_slot_prefix .. item_name
					...
					label_name =
						creative_mode_defines.names.gui.magic_wand_modifier_popup_item_on_ground_count_label_prefix .. item_name
				elseif ghosted_entity_name then
					...
					button_name =
						creative_mode_defines.names.gui.magic_wand_modifier_popup_ghost_entity_name_slot_prefix .. ghosted_entity_name
					...
					label_name =
						creative_mode_defines.names.gui.magic_wand_modifier_popup_ghost_entity_count_label_prefix .. ghosted_entity_name
				elseif ghosted_tile_name then
					-- tile-ghost
					local tile_prototype = game.tile_prototypes[ghosted_tile_name]
					local unit_numbers = data.unit_numbers
					button_name =
						creative_mode_defines.names.gui.magic_wand_modifier_popup_ghost_tile_name_slot_prefix .. ghosted_tile_name
					...
					label_name =
						creative_mode_defines.names.gui.magic_wand_modifier_popup_ghost_tile_count_label_prefix .. ghosted_tile_name
				else
					-- common entity
		...
		-- Title container.
		local actions_frame_title_container =
			actions_frame.add {
			type = "flow",
...
function gui_menu_magicwand.on_gui_click(element, element_name, player, button, alt, control, shift)
	...
	if element.parent and element.parent.name == creative_mode_defines.names.gui.magic_wand_modifier_popup_entities_table then
		if element.type == "sprite-button" then
			...
			check_and_remove_modifier_popup_slot_by_rules(
				element,
				element_name,
				player,
				{
	...
	local clicked, cheat_gui_data, applied_value =
